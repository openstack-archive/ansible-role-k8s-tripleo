...
def get_args():
    ...
    parser.add_argument('--cm-label', action='append', dest='cm_labels',
                        help='Labels to add to the configmap key=value')
...
def main():
    ...
    if not paths:
        LOG.debug("No files to upload for configmap: %s " % args.cm_name)
        return
